...
from policy.common.checkpoint_util import TopKCheckpointManager
from policy.common.json_logger import JsonLogger
from policy.common.pytorch_util import dict_apply, optimizer_to
...
class TrainACTImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: ACTImagePolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: ACTImagePolicy = None
    ...
    def run(self):
        ...
        val_dataloader = DataLoader(val_dataset, collate_fn=dataset.collate_fn, **cfg.val_dataloader)
            ...
            for local_epoch_idx in range(self.epoch, cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        loss = loss_dict['loss'] / cfg.training.gradient_accumulate_every
                        loss.backward()
                        ...
                        step_log.update({
                            ...
                            'train_l1_loss': loss_dict['l1'].item(),
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        if len(val_losses) > 0:
                            ...
                            l1_losses = [loss_dict['l1'] for loss_dict in val_losses]
                            kl_losses = [loss_dict['kl'] for loss_dict in val_losses]
                            total_losses = [loss_dict['loss'] for loss_dict in val_losses]

                            
                            val_l1_loss = torch.mean(torch.tensor(l1_losses)).item()
                            val_kl_loss = torch.mean(torch.tensor(kl_losses)).item()
                            val_total_loss = torch.mean(torch.tensor(total_losses)).item()

                            
                            step_log['val_l1_loss'] = val_l1_loss
                            step_log['val_kl_loss'] = val_kl_loss
                            step_log['val_loss'] = val_total_loss
                ...
                #         a_hat = policy.predict_action(obs_dict)
...
def main(cfg):
    workspace = TrainACTImageWorkspace(cfg)
    workspace.run()
