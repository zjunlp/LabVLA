...
def write_image(image: np.ndarray | PIL.Image.Image, fpath: Path, compress_level: int = 1) -> bool:
    ...
    Returns:
        bool: ``True`` if the image was written successfully, ``False`` if the
            write failed. Callers (e.g. ``add_frame``) must check this and avoid
            recording the image path when it is ``False``, otherwise the dataset
            references a missing/incomplete image file.
...
class AsyncImageWriter:
    ...
    def save_image(
        ...
        # Freeze a private copy before enqueueing. In threading mode the workers
        # share this process's memory, so if the caller mutates the source buffer
        # in place after submitting, the worker could persist a different frame
        # than the one committed for this fpath.
