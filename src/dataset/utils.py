...
def serialize_dict(stats: dict[str, torch.Tensor | np.ndarray | dict]) -> dict:
    ...
    for key, value in flatten_dict(stats).items():
        if isinstance(value, (torch.Tensor | np.ndarray)):
            ...
            # Pass empty lists through unchanged; the typed-element branch below
            # would index value[0] and raise IndexError.
    ...
    for d in dicts:
        for key, value in d.items():
            ...
            if is_vector:
                ...
                # For images/videos and non-1D entries: last-writer-wins is only
                # safe when the colliding definitions are actually identical.
                # Silently overwriting a differing spec (shape/dtype/names) would
                # drop the earlier feature's metadata, so reject the conflict.
                prev = out.get(key)
                if isinstance(prev, dict):
                    prev_sig = (prev.get("dtype"), prev.get("shape"), prev.get("names"))
                    cur_sig = (value.get("dtype"), value.get("shape"), value.get("names"))
                    if prev_sig != cur_sig:
                        raise ValueError(
                            f"Conflicting feature definitions for '{key}': "
                            f"dtype/shape/names {prev_sig} vs {cur_sig}. "
                            f"combine_feature_dicts() only allows last-writer-wins "
                            f"for identical specs."
                        )
                out[key] = value
    return out
...
class Backtrackable(Generic[T]):
    ...
    def __next__(self) -> T:
        # Cursor convention: the *current* item always lives at
        # `self._back_buf[self._cursor - 1]`. `_cursor == 0` => current is the
        # most-recent appended item (`_back_buf[-1]`); each prev() decrements
        # `_cursor`, walking one step older. peek_back()/history()/
        # can_peek_back() all share this `cursor - 1` base.
        #
        # If we've stepped back, replay forward from the back buffer first.
        if self._cursor < 0:
            self._cursor += 1
            return self._back_buf[self._cursor - 1]
    ...
    def prev(self) -> T:
        """
        Step one item back in history and return the PREVIOUS item.

        The current item is `_back_buf[self._cursor - 1]`; prev() walks one
        step older and returns that older item (not the current one), and the
        following next() returns the item we stepped back from.

        Example (source = [0, 1, 2, 3, ...], history >= 3):
            next() -> 0; next() -> 1; next() -> 2   # current is 2
            prev() -> 1                             # previous item
            next() -> 2                             # resume forward
            next() -> 3
