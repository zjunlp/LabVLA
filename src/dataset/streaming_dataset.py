...
from dataset.lerobot_dataset import CODEBASE_VERSION, LeRobotDatasetMetadata
...
from utils.constants import HF_LEROBOT_HOME, LOOKAHEAD_BACKTRACKTABLE, LOOKBACK_BACKTRACKTABLE
...
class StreamingLeRobotDataset(torch.utils.data.IterableDataset):
    ...
    Example:
        Basic usage:
        ```python
        from common.datasets.streaming_dataset import StreamingLeRobotDataset
    ...
    def __init__(
        ...
        if delta_timestamps is not None:
            ...
            # Enforce the integer-frame tolerance BEFORE silently rounding to
            # frame indices, mirroring the map-style path
            # (LeRobotDataset.__getitem__ -> check_delta_timestamps then
            # get_delta_indices). Otherwise a non-integer-frame delta would be
            # snapped via round(d * fps) without any error.
    ...
    def _make_padding_camera_frame(self, camera_key: str):
        """Variable-shape padding frame for the given camera key, in (C, H, W).

        Defensively handles a camera entry in ``meta.info["features"]`` that does
        not declare a NHWC ``shape`` (older info.json schemas, or cameras
        registered without shape metadata).

        Resolution order:
          1. If the camera entry has ``shape``, use it (NHWC -> CHW via permute).
          2. Otherwise fall back to (3, 224, 224) -- the project default that
             downstream image_transforms can resize freely -- and warn once per
             camera_key. The padding frame is masked (``_is_pad=True``) so its
             content never reaches the loss; only its shape must be plausible
             enough to concat with real frames at collate.
        """
        import logging as _logging

        features = (self.meta.info or {}).get("features", {}) or {}
        cam_entry = features.get(camera_key, {}) or {}
        shape = cam_entry.get("shape")

        if shape is not None and len(shape) == 3:
            # Documented contract: NHWC -- (H, W, C) -> permute to (C, H, W).
            return torch.zeros(shape).permute(-1, 0, 1)

        # Defensive fallback. Warn at most once per camera_key per process.
        warned = getattr(type(self), "_low_v5_08_warned_cams", None)
        if warned is None:
            warned = set()
            type(self)._low_v5_08_warned_cams = warned
        if camera_key not in warned:
            warned.add(camera_key)
            _logging.getLogger(__name__).warning(
                "[streaming] camera '%s' has no NHWC 'shape' in features metadata "
                "(LOW-V5-08 fallback) -- using default (3, 224, 224) padding frame. "
                "Please add 'shape' to meta/info.json for this camera; padding is "
                "masked via _is_pad so loss is unaffected, but downstream collate "
                "may still expect a specific resolution.",
                camera_key,
            )
    ...
    def _get_video_frame_padding_mask(
    ...
    ) -> tuple[dict[str, torch.BoolTensor], dict[str, torch.Tensor]]:
        """Build the `_is_pad` mask AND the matching frame stack.

        Out-of-bounds delta timestamps are clamped to the episode boundary in
        ``_get_query_timestamps``, so ``_query_videos`` decodes a duplicate
        boundary frame for those positions. Rebuild the per-key frame stack so
        masked positions carry a real zero pad frame instead, keeping the tensor
        consistent with its ``_is_pad`` mask and ordered by
        ``original_timestamps``.
        """
        padding_mask = {}
        padded_frames = {}
        ...
        for video_key, timestamps in original_timestamps.items():
            ...
            # Stack in original-timestamp order so consumers see explicit pad,
            # not duplicate edge frames.
            if frames:
                padded_frames[video_key] = (
                    torch.stack(frames) if len(frames) > 1 else frames[0]
                )

        return padding_mask, padded_frames
    ...
    def make_frame(self, dataset_iterator: Backtrackable) -> Generator:
        ...
        if len(self.meta.video_keys) > 0:
            ...
            if self.image_transforms is not None:
                ...
                for cam in image_keys:
                    ...
                    if callable(apply_seeded):
                        video_frames[cam] = apply_seeded(
                            video_frames[cam],
                            seed_parts=(self.repo_id, ep_idx, cam, float(current_ts)),
                        )
                    else:
            ...
            if self.delta_indices is not None:
                # Always return the same number of frames; unavailable ones are padded.
                padding_mask, padded_frames = self._get_video_frame_padding_mask(
                    video_frames, query_timestamps, original_timestamps
                )
                # Use real pad frames (not clamped duplicate boundary frames) so
                # the tensor and its mask agree.
                updates.append(padded_frames)
