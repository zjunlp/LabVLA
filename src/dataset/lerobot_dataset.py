...
from dataset.compute_stats import aggregate_stats, compute_episode_stats
from dataset.image_writer import AsyncImageWriter, write_image
    ...
    def _save_episode_metadata(self, episode_dict: dict) -> None:
        ...
        if self.latest_episode is None:
            ...
            if Path(latest_path).exists():
                ...
                # Frame count = dataset_to_index - dataset_from_index. (Using
                # episode_index here would be an episode ordinal, not a frame
                # count, giving a wrong av_size_per_frame and chunk-rollover
                # threshold.)
                latest_num_frames = (
                    self.latest_episode["dataset_to_index"][0]
                    - self.latest_episode["dataset_from_index"][0]
                )
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # When only a subset of episodes is loaded, map absolute_index ->
        # relative_index_in_filtered_dataset.
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.image_transforms is not None:
            ...
            # Route through apply_with_seed() for deterministic, resume-stable
            # augmentation (seeded by stable sample metadata), matching the
            # v21/v30 adapter paths. Falls back to the bare call when
            # image_transforms is a plain callable without apply_with_seed.
            frame_idx = item["frame_index"].item() if "frame_index" in item else idx
            apply_seeded = getattr(self.image_transforms, "apply_with_seed", None)
            for cam in image_keys:
                if callable(apply_seeded):
                    item[cam] = apply_seeded(
                        item[cam], seed_parts=(self.repo_id, ep_idx, cam, frame_idx)
                    )
                else:
    ...
    def _save_image(
        ...
        """Write an image frame to disk.

        Returns:
            bool: ``True`` on a confirmed successful write (synchronous path) or
            when the write was successfully *enqueued* on the async image
            writer. ``False`` only when a synchronous write failed. ``add_frame``
            uses this to avoid recording a path for a frame whose synchronous
            write failed.
        """
        if self.image_writer is None:
            if isinstance(image, torch.Tensor):
                image = image.cpu().numpy()
            return write_image(image, fpath, compress_level=compress_level)
        else:
            # Async path: the actual disk write happens later in a worker, so a
            # write failure cannot be observed synchronously here. Enqueue
            # success is reported as True; write_image() still logs failures.
    ...
    def add_frame(self, frame: dict) -> None:
        ...
        for key in frame:
            ...
            if self.features[key]["dtype"] in ["image", "video"]:
                ...
                ok = self._save_image(frame[key], img_path, compress_level)
                # Do not record a path for an image that failed to write. Silently
                # skipping the append would desync this column from the others, so
                # fail loudly instead.
                if not ok:
                    raise RuntimeError(
                        f"Failed to write image for key '{key}' at {img_path}; "
                        f"aborting add_frame to avoid recording a missing image path."
                    )
