...
from dataset.utils import load_image_as_numpy
...
class RunningQuantileStats:
    ...
    def _adjust_histograms(self):
        ...
        for i in range(len(self._histograms)):
            ...
            # New edges with small padding to ensure range coverage.
            padding = (self._max[i] - self._min[i]) * 1e-10
            new_edges = np.linspace(
                self._min[i] - padding, self._max[i] + padding, self._num_quantile_bins + 1
            )

            # Redistribute existing counts by mapping each old bin center
            # into the new bins.
...
def _prepare_array_for_stats(array: np.ndarray, axis: int | tuple[int, ...] | None) -> tuple[np.ndarray, int]:
    ...
    if axis == (0, 2, 3):  # Image data
        ...
        return reshaped, batch_size * height * width
...
def get_feature_stats(
    ...
    if reshaped.shape[0] < 2:
        ...
        # Forward the caller's quantile_list so the >= 2 samples branch honors
        # custom quantiles instead of falling back to RunningQuantileStats's
        # DEFAULT_QUANTILES.
...
def aggregate_feature_stats(stats_ft_list: list[dict[str, dict]]) -> dict[str, dict[str, np.ndarray]]:
    ...
    # Cross-dataset quantile aggregation via weighted-mean is mathematically
    # wrong (weighted mean of per-dataset quantiles != quantile of the union),
    # producing silently-incorrect q01/q99 that poison q01_q99 normalization.
    # Drop the quantile keys with a one-time warning; the correct path is to
    # recompute q01/q99 via RunningStats over the merged data in a single exact
    # pass (aggregate_datasets() does this against the union).
    if stats_ft_list:
        quantile_keys = [
            k for k in stats_ft_list[0]
        ...
        if quantile_keys:
            try:
                ...
                warn_once(
                    ...
                    ("aggregate_stats_drops_quantiles",),
                    "[aggregate_stats] dropping quantile keys "
                    f"{quantile_keys} — cross-dataset quantile aggregation via "
                    "weighted-mean is mathematically wrong. Recompute q01/q99 "
                    "from the merged dataset via data_process.stats.compute "
                    "(uses RunningStats incremental aggregation).",
                )
            except Exception:
                # warn_once is best-effort; skip if utils.logging_utils is not
                # importable (e.g. some test environments).
                pass
        # Intentionally do NOT write q-keys into `aggregated`.
