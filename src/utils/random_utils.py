...
from dataset.utils import flatten_dict, unflatten_dict
from utils.constants import RNG_STATE
...
def serialize_torch_rng_state() -> dict[str, torch.Tensor]:
    ...
    Serializes ALL CUDA devices (one per-device key), matching the main
    checkpoint path so a process touching multiple CUDA generators restores
    reproducibly.
    """
    torch_rng_state_dict = {"torch_rng_state": torch.get_rng_state()}
    if torch.cuda.is_available():
        cuda_states = torch.cuda.get_rng_state_all()
        torch_rng_state_dict["torch_cuda_rng_state_count"] = torch.tensor(
            [len(cuda_states)], dtype=torch.int64
        )
        for i, state in enumerate(cuda_states):
            torch_rng_state_dict[f"torch_cuda_rng_state_{i}"] = state
...
def deserialize_torch_rng_state(rng_state_dict: dict[str, torch.Tensor]) -> None:
    ...
    Restores ALL CUDA devices via ``torch.cuda.set_rng_state_all()``;
    backwards-compatible with old single-device checkpoints
    (``torch_cuda_rng_state`` key).
    """
    torch.set_rng_state(rng_state_dict["torch_rng_state"])
    if not torch.cuda.is_available():
        return
    if "torch_cuda_rng_state_count" in rng_state_dict:
        n = int(rng_state_dict["torch_cuda_rng_state_count"].item())
        cuda_states = [
            rng_state_dict[f"torch_cuda_rng_state_{i}"] for i in range(n)
        ]
        torch.cuda.set_rng_state_all(cuda_states)
    elif "torch_cuda_rng_state" in rng_state_dict:
        # Legacy single-device checkpoint.
...
def seeded_context(seed: int) -> Generator[None, None, None]:
    ...
    try:
        ...
        # Restore RNG even on exception, else the ctx-seeded state leaks into
        # the caller and breaks determinism.
