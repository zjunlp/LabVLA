...
def init_logging(
    ...
    if is_main_process:
        ...
        # Suppress console output for non-main processes WITHOUT raising the
        # logger level (that would filter records before the file handler sees
        # them, breaking "console suppressed but file still works"). Just skip
        # the console handler; NullHandler avoids the "no handlers" fallback.
...
def format_big_number(num: float, precision: int = 0) -> str:
    """Render a big number with a human-friendly SI suffix.

    ``precision`` controls the number of fractional digits on the formatted
    value (e.g. ``precision=1`` → ``"2.5M"``, ``precision=0`` → ``"2M"``).
    The overflow branch (>= 1e18) still returns a str honoring ``precision``.
    """
    suffixes = ["", "K", "M", "B", "T", "Q"]
    divisor = 1000.0
    ...
    # Ran past "Q" (10^18+). Still format with the last suffix so the return
    # type is always str and `precision` is always honored.
    return f"{num * divisor:.{precision}f}{suffixes[-1]}"
