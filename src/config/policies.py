...
from huggingface_hub.constants import CONFIG_NAME as _HF_CONFIG_NAME
...
class PreTrainedConfig(draccus.ChoiceRegistry, HubMixin, abc.ABC):  # type: ignore[misc,name-defined] #TODO: draccus issue
    ...
    # Override-able config-file name (default "config.json"); subclasses can set
    # e.g. "train_config.json" without monkeypatching the HF module.
    CONFIG_NAME: str = _HF_CONFIG_NAME
    ...
    def __post_init__(self) -> None:
        if not self.device or not is_torch_device_available(self.device):
            ...
            rank_zero_warn_once(
                ...
                "Device '%s' is not available. Switching to '%s'.",
    ...
    def _save_pretrained(self, save_directory: Path) -> None:
        with open(save_directory / self.CONFIG_NAME, "w") as f, draccus.config_type("json"):
        ...
        **policy_kwargs: Any,
        ...
        if Path(model_id).is_dir():
            if cls.CONFIG_NAME in os.listdir(model_id):
                ...
                logger.error(f"{cls.CONFIG_NAME} not found in {Path(model_id).resolve()}")
        else:
            try:
                ...
                raise FileNotFoundError(
                    f"{cls.CONFIG_NAME} not found on the HuggingFace Hub in {model_id}"
                ) from e

        # Fail loud BEFORE parse so a missing config file yields a clean
        # FileNotFoundError instead of a confusing draccus traceback.
        if config_file is None:
            raise FileNotFoundError(f"{cls.CONFIG_NAME} not found in {model_id}")
